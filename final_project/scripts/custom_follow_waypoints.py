...
class GetPath(State):
    ...
    def execute(self, userdata):
        ...
        rospy.loginfo("Waiting to recieve serving robot's waypoints!")
        ...
        while not self.path_ready:
            ...
            rospy.loginfo("Received waypoint %d" % waypoint_number) 
...
class FollowPath(State):
    ...
    def execute(self, userdata):
        ...
        rospy.loginfo("############time recording start!##############")
        ...
        for waypoint in waypoints:
            ...
            current_position = (waypoint.pose.pose.position.x, waypoint.pose.pose.position.y)
            if current_position == (waypoints[1].pose.pose.position.x, waypoints[1].pose.pose.position.y):
                for i in range(5):  
                    rospy.sleep(1)
                    rospy.loginfo("getting food...%ds" % (i+1)) 
                                
            elif(current_position == (waypoints[2].pose.pose.position.x, waypoints[2].pose.pose.position.y)):
                rospy.loginfo("To start serving at table_3")
                for i in range(2):  
                    rospy.sleep(1)
                    rospy.loginfo("serving food...%ds" % (i+1)) 
                rospy.loginfo("serving at table_3 is complete")

            elif(current_position == (waypoints[3].pose.pose.position.x, waypoints[3].pose.pose.position.y)):
                rospy.loginfo("To start serving at table_4")
                for i in range(2):  
                    rospy.sleep(1)
                    rospy.loginfo("serving food...%ds" % (i+1)) 
                rospy.loginfo("serving at table_4 is complete")
            
            elif(current_position == (waypoints[4].pose.pose.position.x, waypoints[4].pose.pose.position.y)):
                rospy.loginfo("To start serving at table_5")
                for i in range(2):  
                    rospy.sleep(1)
                    rospy.loginfo("serving food...%ds" % (i+1)) 
                rospy.loginfo("serving at table_5 is complete")
                      
          
            rospy.loginfo("Starting movement to the next waypoint (%s, %s)" %
